...
namespace Zenject
{
    public class SceneContext : RunnableContext
    {
        ...
        public event Action PostKernelInitialize
        {
            add { SceneKernel.PostInitialize += value; }
            remove { SceneKernel.PostInitialize -= value; }       
        }
        ...
        private SceneKernel SceneKernel { get; set; } 
        public bool IsKernelCreated => SceneKernel;
        public bool KernelWasInitialize => IsKernelCreated && SceneKernel.WasInitialize;
        ...
        void InstallBindings(List<MonoBehaviour> injectableMonoBehaviours)
        {
            ...
            _container.Bind(typeof(SceneKernel), typeof(MonoKernel))
                .To<SceneKernel>().FromNewComponentOn(gameObject).AsSingle().OnInstantiated((context, kernel) => SceneKernel = kernel as SceneKernel).NonLazy();
